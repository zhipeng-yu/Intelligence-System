...
生产部署 `8cc8a54f` 使用源码提交 `aed9c23`，远端 D1 已应用到 `0005`。生产 Secret 已配置，`ADMIN_KEY` 已轮换并随当前部署生效；真实只读 Edge 检索已验收。访问预算优化已在源码完成但尚未部署，新工作器计划任务当前保持禁用。
...
已注册的任务计划每分钟启动一次，`MultipleInstances IgnoreNew`，但优化期间已按用户授权临时禁用。API 用独立工作器密钥、50 分钟租约和 claim token 控制认领与回传，D1 条件更新保证服务端全局最多一个运行任务；过期任务不重领。同一 payload 可安全重复提交。
...
自动化测试不访问真实小红书、不调用真实 AI。既有真实只读验收任务与新计划任务首次自动触发均为 `completed`，无安全验证；首次自动任务处理 2 个账号，0 条命中、0 个失败。旧七日任务已禁用但未删除，旧状态、`seen.json` 和 `held_candidates` 保持原样。本轮优化尚未执行生产 `0006`、Pages 部署或计划任务恢复。

## 待授权上线

当前生产尚未包含 `0006`、预算与可观测性 API、工作器漏斗或页面展示改动。第一阶段仍只使用系统 Edge 和每账号 20 条候选，不做签名逆向、网络响应监听、代理、Cookie/IP 轮换、并发或完整内容持久化。生产 D1 迁移、Pages 部署、计划任务恢复与真实只读验收均需用户另行明确授权，详见 `new-window-prompt.md`。
