...
自动化测试不访问真实小红书、不调用真实 AI。首次生产验收使用“学而思网校”（小红书号 `27247756272`），账号核验为 `ready`；关键词“学习”、近 7 日检索为 `completed / candidates_exhausted`，读取 20 条主页候选，窗口内图文、详情打开和命中均为 0。再次筛选时登录失效，阻塞发生前没有创建新的核验或搜索任务；Windows 通知已发送，本地已停机，D1 两类活动任务均为 0。旧七日任务已禁用但未删除，旧状态、`seen.json` 和 `held_candidates` 保持原样。
