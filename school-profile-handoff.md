...
用户已批准下一窗口按 ponytail full 实施最小优化，当前生产尚未包含：任务漏斗计数、按实际执行日控制并展示的详情实际/预留/剩余预算、50 分钟租约、过期任务不自动重领、服务端全局单运行、跨账号候选汇总与标题优先、30 条早停和更完整的安全阻断。第一阶段继续使用系统 Edge 和每账号 20 条候选，不做签名逆向、网络响应监听、代理、Cookie/IP 轮换、并发或完整内容持久化。执行与上线授权边界见 `new-window-prompt.md`。
