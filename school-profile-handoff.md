...
当前 `main` 已加入“小红书号”异步核验和 `0007`，仅完成本地验证；生产 D1、Pages 和计划任务尚未切换到这版源码。
...
已注册的任务计划每分钟启动一次，`MultipleInstances IgnoreNew`，当前为 `Ready`。API 用独立工作器密钥、50 分钟租约和 claim token 控制认领与回传，D1 条件更新保证账号核验和检索合计全局最多一个运行任务；旧工作器不会收到核验任务，过期任务不重领，同一 payload 可安全重复提交。
...
当前源码已通过三组 Node 测试（8、9、12 项）、两组 Python 测试（9、15 项）、全新本地 `0001`～`0007` 迁移和 Pages Functions 构建。GPT 内置浏览器已完成小红书号添加与状态展示、1280px 桌面、390px、主导航、键盘焦点和控制台验收，截图已更新。
...
生产已包含 `0006`、预算与可观测性 API、工作器漏斗和页面展示改动；尚未包含 `0007` 和小红书号核验。上线需另行授权。第一阶段仍只使用系统 Edge 和每账号 20 条候选，不做签名逆向、网络响应监听、代理、Cookie/IP 轮换、并发或完整内容持久化。
