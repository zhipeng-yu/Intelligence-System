...
用户已要求按 ponytail full 优化网络资料工作器。交接前生产 D1 聚合状态为 `queued=0`、`running=0`、`blocked=0`；现有功能仍按原设计运行，优化尚未写入代码或生产。

下一窗口只实施最小高收益阶段：D1 聚合漏斗与 180 次/上海自然日的详情预留预算、50 分钟租约、过期任务不自动重领、服务端全局单运行、全部账号候选先汇总去重排序、标题优先但不排除、30 条早停、安全异常统一 blocked，以及页面显示单任务和今日实际/预留/剩余详情数。第一阶段不做网络响应监听、签名逆向、代理、Cookie/IP 轮换、并发、超过 20 条的分页或任何正文/临时参数持久化。完整执行说明见 `new-window-prompt.md`。

本机新计划任务直接从仓库运行工作器。修改 `automation/` 前必须先向用户说明并取得临时禁用授权；完成后也不得自行恢复。代码优化授权不包含生产 D1 迁移、Pages 部署、计划任务变更或真实 Edge 检索。
