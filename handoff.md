...
- 当前 `main` 已加入“小红书号”异步核验源码和 `0007_resolve_red_ids.sql`，但生产仍是上述 `0006`/`4ea51be` 基线；本轮未迁移生产 D1、部署 Pages、调整计划任务或执行真实小红书核验。
...
- 生产 D1 `0006`、Pages 部署和现有计划任务仍保持 2026-09-03 状态。上线小红书号核验必须另行获得授权，并按 `0007` 迁移、Pages 部署、工作器源码切换、真实只读验收的顺序执行。
...
2026-09-03 已按“D1 迁移 → Pages 部署 → 生产只读核对 → 恢复既有计划任务”的顺序完成原 MVP 上线。2026-09-08 的小红书号核验仅完成源码、迁移和本地验证，尚未上线。未修改 Secret、未创建真实检索、未调用真实 AI、未删除旧任务或历史候选，也未执行任何远端删除。
