...
- AI 改为读取当前画像与新资料，输出受影响卡片的完整新版短条目；新上传资料优先，保留不冲突旧条目，替换冲突项、删除过时项并去重。
- 每张卡片不设最低条数，最多 12 条，总长度仍不超过 4000 字；未变化的卡片不写历史、不增加来源。
- 删除人工编辑、解锁和运行时锁定逻辑。
- 复用 `profile_history` 形成全局线性撤销栈：一次资料更新多张卡片仍只算一步，只能撤销最新一步，无重做、分支或任意版本跳转。
- 只有实际改变卡片且尚未撤销的资料进入卡片级来源；撤销时内容和来源一起回退。
- 失败、未生效或已经按顺序撤销的资料可以彻底删除 R2 对象和 D1 文件身份；画像历史永久保留但不保留已删除文件身份。
- ponytail full 约束：不新增完整快照表、独立来源表、框架、依赖、队列或后台任务。具体实施顺序与验收项见 `new-window-prompt.md`。
