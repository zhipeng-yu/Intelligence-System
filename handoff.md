...
- `Ledu-Network-Materials-Worker` 使用提交 `4162d08` 的源码，已恢复为 `Ready`，每分钟触发且 `MultipleInstances IgnoreNew`。验收后的空闲轮询返回 0，本地与 D1 均未停机。
...
- 生产 D1、Pages 和本机工作器均已切换到 `0007`/`4162d08`；账号核验与检索均无活动任务，`network_worker_control.halted = 0`。
