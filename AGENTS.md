...
- 完成任务前检查并按实际结果精简更新 `readme.md`、`handoff.md`、相关专项交接文档和本文件；删除失效说明，不追加流水账。内容仍准确时无需为了制造改动而重写。
- 除非用户明确要求不提交或不推送，验证通过后只暂存本次文件，提交到本地 `main`，重新只读核对远端后正常快进推送 `origin/main`。推送代码不等于授权生产部署或远端迁移。
