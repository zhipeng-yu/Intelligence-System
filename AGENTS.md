...
小规模多用户网络资料 MVP、访问预算优化与“小红书号”异步核验均已上线；生产部署 `5df53e87` 运行源码 `4162d08`，远端 D1 已到 `0007_resolve_red_ids.sql`，既有 Secret 未修改。`ADMIN_KEY` 已于 2026-09-02 轮换并随当前部署生效。真实只读账号核验与检索已验收；`Ledu-Network-Materials-Worker` 使用同一源码，已注册为每分钟 `IgnoreNew`、状态 `Ready`，验收后空闲轮询返回 0，D1 与本地均未停机。

旧 `Ledu-Xiaohongshu-Course-Trial` 已禁用但未删除；旧 `seen.json`、运行状态和 `held_candidates` 已原样保留，禁用前后文件哈希一致。不得删除历史候选。

网络资料效率、访问预算、任务可观测性和小红书号核验已在生产生效；实现与验证结果见 `new-window-prompt.md`。生产验收账号为“学而思网校”、小红书号 `27247756272`；关键词“学习”、近 7 日的检索读取 20 条主页候选，窗口内图文、详情打开和命中均为 0，任务正常完成且未触发停机。
